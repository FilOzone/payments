#! /bin/bash
# deploy-mainnet deploys the Payments contract to Filecoin Mainnet
...
#
echo "Deploying Payments to mainnet"
...
PAYMENTS_IMPLEMENTATION_ADDRESS=$(forge create --rpc-url "$RPC_URL" --keystore "$KEYSTORE" --password "$PASSWORD" --broadcast --nonce $NONCE --chain-id 314 src/Payments.sol:Payments | grep "Deployed to" | awk '{print $3}')
...
PAYMENTS_PROXY_ADDRESS=$(forge create --rpc-url "$RPC_URL" --keystore "$KEYSTORE" --password "$PASSWORD" --broadcast --nonce $NONCE --chain-id 314 src/ERC1967Proxy.sol:PaymentsERC1967Proxy --constructor-args $PAYMENTS_IMPLEMENTATION_ADDRESS $INIT_DATA | grep "Deployed to" | awk '{print $3}')
